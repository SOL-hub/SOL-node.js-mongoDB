    ...
    function (inputValueId, inputValuePw, done) {
      db.collection("login").findOne(
        ...
        function (error, result) {
          ...
          if (!result)
            return done(null, false, { message: "존재하는 아이디확인완료" });
...
app.post("/add", function (req, res) {
  ...
  db.collection("counter").findOne(
    ...
    function (error, result) {
      ...
      const willbeSave = {
        _id: totalPostCounter + 1,
        writer: req.user._id,
        title: req.body.title,
        date: req.body.date,
      };

      db.collection("post").insertOne(willbeSave, function (error, result) {
        console.log("저장완료2");
...
const storage = multer.diskStorage({
  //램에다 저장하고 싶다면 diskStorage말고 memoryStorage()로
